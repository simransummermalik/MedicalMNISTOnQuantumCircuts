...
Medical images contain hundreds of pixel values, but small quantum models accept only a few inputs, so images must be compressed before training. If useful detail is lost at this step, poor performance may be blamed on a quantum model that never received the information. This study tests whether model responses to image details survive compression. Chest X-ray, breast ultrasound, and colon tissue images from MedMNIST were reduced to 4–64 numbers using principal component analysis (PCA), a method that summarizes major pixel patterns. Full-image and compressed models received the same controlled edits: covered regions, blurred texture and edges, reduced contrast, and noise. A preservation score compared the resulting changes in predicted probability. With four numbers, PneumoniaMNIST reached 84.4% balanced accuracy, which gives each class equal weight, but its response to center coverage fell to 8.4% of the full-image response. PathMNIST kept 62.2% of pixel variation, while texture and edge responses fell to about 1%. A four-qubit model reached 69.2% balanced accuracy versus 85.8% for a non-quantum model using identical inputs. Uncertainty estimates and 50 random-label tests show that the quantum result remains preliminary. By separating information lost during compression from limitations of the later model, this approach reduces the risk of misleading quantum-model comparisons and shows what information reached the model. It does not claim quantum advantage or clinical use.

## Research question

The project asks:

**When a medical image is reduced to the small number of inputs accepted by a quantum model, which model responses survive that reduction?**
